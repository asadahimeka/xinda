            ...
            <div>
                <div class="siftingHeader">产品类型</div>
                    <ul class="produclist font14" v-for="(item,i) in prodType">
                        <li><a :class="{active:i==proi}" @click="proc(i,item.code)">{{item.name}}</a></li>
                    </ul>
            </div>
        </div>
        <div class="Shopboxtitle font14">
            <ul v-for = "(item,i) in shopSort">
                <li><a :class="{active:i==sori}" @click="sorc(i,item.sort)">{{item.name}}&nbsp;<i class="iconfont">&#xe731;</i></a></li>
            </ul>
        </div>
        <!-- 内容区 -->
            ...
            <template v-for="info in shopinfo">
                <div class="shopBox" :key="info.id">
                    ...
                    <div class="boxright">
                        ...
                        <div>累计服务客户次数:{{info.orderNum}}<span class="line">|</span>好评率:{{rate(info.goodJudge,info.totalJudge)}}</div>
