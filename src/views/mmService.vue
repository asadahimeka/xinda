<template>
    <div class="service container">
        <div class="moduleTitle">
            <h3><a :class="{active : show}" @click = "getProvider()">推荐服务商</a></h3>
            <h3><a  :class="{active : !show}" @click = "getProduct()">推荐服务</a></h3>
        </div>
        <div class="moduleBody">
                <a v-for = "list in providerList" v-if="show" @click="enter(list.id)">
            ...
            <template v-for = "list in productList" v-if="!show">
                <a @click="enter(list.id)">
                    <div><img :src="providerImg(list.providerImg)" alt=""></div>
                    <h3>{{list.serviceName}}</h3>
                    <h4>{{list.providerName}}</h4>
                    <div class="price"><span>￥{{list.price/100}}</span>元</div>
                    <button>查看详情</button>
                </a>
            </template>
...
<script>
    export default {
        ...
        methods: {
            getProduct() {
                this.ajax.post( '/recommend/list', {} ).then( (data) => {
